...
use Dnoegel\DatabaseInspection\RouteProvider\ShopwareRouteProvider;
use Dnoegel\DatabaseInspection\SqlProblemInspector;
use Dnoegel\DatabaseInspection\Storage\JsonStorage;
 ...
 */
class Shopware_Plugins_Frontend_DsnDatabaseInspection_Bootstrap extends Shopware_Components_Plugin_Bootstrap
{
    ...
    public function onStartDispatch(Enlight_Event_EventArgs $args)
    {
        ...
        $parser = new SqlProblemInspector(
            new JsonStorage(), new ShopwareRouteProvider(), new DebugTrace(), true
        );

        $masterAdapter = $container->get('db');
        $masterConfig  = $container->getParameter('shopware.db');
        $container->set('db', new \Dnoegel\DatabaseInspection\ShopwareDbDecorator($masterConfig, $masterAdapter, $parser));
